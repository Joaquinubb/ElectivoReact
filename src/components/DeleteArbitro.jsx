...
export const DeleteArbitro = ({ arbitro, setRefresh, refresh }) => {
  const name = arbitro.nombre_arbitro + " " + arbitro.apellido_arbitro;
  const id = arbitro.id_arbitro;
  const [error, setError] = useState(null);

  const deleteArbitro = async () => {
    const apiUrl = process.env.REACT_APP_API;
    try {
      const response = await fetch(`${apiUrl}/arbitros/${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        setRefresh(!refresh);
        ...
        setError("Error al borrar el árbitro");
      }
    } catch (error) {
          ...
          <div className="modal-content red-text border-red-2">
            ...
            <div className="d-flex flex-column w-100 px-3">
              <p className="text-center bold text-20">
                ¿Está seguro que desea borrar el árbitro {name}?
              </p>
