...
export const DeleteJugador = ({ jugador, setRefresh, refresh }) => {
  const name = jugador.nombre_jugador + " " + jugador.apellido_jugador;
  const id = jugador.id_jugador;
  const [error, setError] = useState(null);

  const deleteJugador = async () => {
    const apiUrl = process.env.REACT_APP_API;
    try {
      const response = await fetch(`${apiUrl}/jugadores/delete/?id=${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        setRefresh(!refresh);
          ...
          <div className="modal-content red-text border-red-2">
            ...
            <div className="d-flex flex-column w-100 px-3">
              <p className="text-center bold text-20">
                ¿Está seguro que desea borrar el jugador {name}?
              </p>
